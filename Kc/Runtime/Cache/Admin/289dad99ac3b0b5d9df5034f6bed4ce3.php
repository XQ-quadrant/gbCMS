...
<body id="login">
  ...
  <div class="app-cam">
	  <form>
		...
		<div class="submit"><input type="button" onClick="login()" value="登  录"></div>
...
<script>
    ...
    function login(){
        var countname = $('#count').val();
        var password = $('#password').val();

        $.ajax({

            url: '/kechuang4/index.php/Admin/Index/login',
            type: 'post',//提交的方式
            dataType:'json',
            data: {'name':countname,'password':password},//myForm.serializeArray(),
            success: function(data) {
                //var msg1 =evalJSON(msg);
                //alert(msg1['status']);
                //var aa = jQuery.parseJSON(data);
                if(data.status==1){
                    location.href = '/kechuang4/index.php/Admin/Index/index';
                }else {
